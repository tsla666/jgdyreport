...
def is_within_trading_days(date_str):
    try:
        ...
        # 检查目标日期是否在最近2个交易日内
        return target_date.date() in trading_days
...
def crawl_research_data():
    ...
    for institution, items in institution_data.items():
        deduplicated_items = []
        for item in items:
            company_key = f"{item['name']}_{item['code']}"
            if company_key not in seen_companies:
                seen_companies.add(company_key)
                deduplicated_items.append(item)
        if deduplicated_items:
            deduplicated_data[institution] = deduplicated_items
    
    # 输出去重后的结果
    total_count = 0
    for institution, data in deduplicated_data.items():
        count = len(data)
        total_count += count
    ...
    print(f"\n爬取完成，去重后总计获取 {total_count} 条符合条件的调研记录")
